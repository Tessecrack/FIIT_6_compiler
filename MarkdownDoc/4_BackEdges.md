...
#### Практическая часть
Поиск обратных рёбер и анализ графа на приводимость реализованы в классе ```BackEdges```. 
...
Кроме того, был написан метод расширения для класса ```List<Edge>```, который в цикле проверяет, содержит ли список искомое ребро или нет. 
